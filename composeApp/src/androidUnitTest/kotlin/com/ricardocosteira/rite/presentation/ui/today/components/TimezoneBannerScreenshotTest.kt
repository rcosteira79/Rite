package com.ricardocosteira.rite.presentation.ui.today.components

import androidx.compose.ui.test.junit4.createComposeRule
import androidx.compose.ui.test.onRoot
import com.github.takahirom.roborazzi.captureRoboImage
import com.ricardocosteira.rite.presentation.ui.theme.RiteThemeFallback
import org.junit.Rule
import org.junit.Test
import org.junit.runner.RunWith
import org.robolectric.RobolectricTestRunner
import org.robolectric.annotation.Config
import org.robolectric.annotation.GraphicsMode

@RunWith(RobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(
    sdk = [33],
    qualifiers = "w360dp-h800dp-420dpi",
    application = android.app.Application::class
)
class TimezoneBannerScreenshotTest {

    @get:Rule
    val composeRule = createComposeRule()

    @Test
    fun with_previous_timezone_light() {
        composeRule.setContent {
            RiteThemeFallback(darkTheme = false) {
                TimezoneBanner(previousTimezone = "Europe/London", onDismiss = {})
            }
        }
        composeRule.onRoot().captureRoboImage()
    }

    @Test
    fun with_previous_timezone_dark() {
        composeRule.setContent {
            RiteThemeFallback(darkTheme = true) {
                TimezoneBanner(previousTimezone = "Europe/London", onDismiss = {})
            }
        }
        composeRule.onRoot().captureRoboImage()
    }

    @Test
    fun no_previous_timezone() {
        composeRule.setContent {
            RiteThemeFallback(darkTheme = false) {
                TimezoneBanner(previousTimezone = null, onDismiss = {})
            }
        }
        composeRule.onRoot().captureRoboImage()
    }
}
