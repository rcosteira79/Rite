...
class TodayViewModelSwipeTest {
    ...
    fun `undoDelete cancels pending delete and restores habit to state`() = runTest {
        ...
        try {
            ...
            viewModel.deleteHabit(inputHabitId)
            // Wait for the reactive pipeline to filter the habit out of daily before undoing.
            ...
            // habit back in daily because _pendingDelete = null re-runs the combine filter).
            val actualState = viewModel.state.first { state ->
                state.pendingDelete == null && state.daily.any { it.habitId == inputHabitId }
            }
            assertNull(
                actualState.pendingDelete,
            ...
            assertTrue(
                actualState.daily.any { it.habitId == inputHabitId },
