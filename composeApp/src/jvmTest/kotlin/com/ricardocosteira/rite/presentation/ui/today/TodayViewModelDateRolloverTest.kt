    ...
    fun `state recomposes for the new date when CurrentDateProvider emits day rollover`() =
        runTest {
            ...
            try {
                ...
                assertTrue(
                    yesterdayState.daily.any { it.instanceId == "i-yesterday" },
                    "Expected yesterday's PENDING instance in daily before rollover"
                )
                ...
                val newState = viewModel.state.first { state ->
                    state.daily.any { it.instanceId == "i-today" }
                }
                assertTrue(
                    newState.daily.none { it.instanceId == "i-yesterday" },
                    "Expected yesterday's daily instance to disappear after rollover"
                )
                assertEquals(
                    "i-today",
                    newState.daily.firstOrNull()?.instanceId,
                    "Expected today's instance in daily after rollover"
                )
            } finally {
